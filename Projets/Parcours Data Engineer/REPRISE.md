...
## Diagnostic de reprise après la coupure d'août (01/09/2026)

Un mois sans pratique (vacances d'août) : quiz de 15 questions couvrant S1 → S7 pour faire un état des lieux avant de reprendre. Résultat : 7/15 correctes du premier coup.

**Solide malgré la coupure :** INNER JOIN, `PARTITION BY` vs `GROUP BY`, `self` vs variable locale (OOP), modélisation en étoile (placement d'un attribut de dimension), `if_exists="replace"`, `drop_duplicates()` (nécessité de réaffecter), opérateur `>>` Airflow.

**À retravailler avant de reprendre S7 (détail et priorités dans `fiches-revision/quiz_journal.md`) :**
- `.loc[]` vs `.iloc[]` inversés dans la réponse (priorité haute, c'est une base) — corrigé : `.loc` = étiquette/index, `.iloc` = position entière.
- `KeyError` sur clé de dict absente (S1) — oublié.
- Piège `merge(how="inner")` : perte silencieuse de lignes (S4) — oublié.
- Alias obligatoire sur une table dérivée en PostgreSQL (S2) — raison confondue avec celle d'une CTE.
- `.copy()` avant modification d'un DataFrame filtré (S5, `SettingWithCopyWarning`) — oublié.
- CTE vs sous-requête imbriquée : avantage réel = lisibilité + chaînage, pas une histoire de taille d'opération (S4) — à reformuler.
- Java `new` pour instancier un objet (S7) — oublié.
- Spark lazy evaluation, transformation vs action (S7) — oublié.

**Plan proposé :** reprendre ces 8 points un par un (le plus ancien et le plus basique d'abord : `.loc`/`.iloc`, puis merge/CTE/copy, puis Java/Spark) avant de relancer la Semaine 7 (mini-projet cumulatif + démarrage des candidatures).
